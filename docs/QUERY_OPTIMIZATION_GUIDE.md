...
Every `lib/ysc` module with Ecto queries should define `ci_query_explain_query/0` (see `docs/QUERY_EXPLAIN_CI.md`). The registry in `priv/ci/query_explain_targets.exs` is built from all discoverable `*_query` / `base_query` functions under `lib/ysc`. Any PR touching `lib/ysc` runs the full `Ysc.*` target set.

When adding or changing a query, update `ci_query_explain_query/0` (or related `*_query` helpers) and keep execution (`Repo.all`, preloads, side effects) in separate functions.
