defmodule Ysc.RateLimitCache do
  ...
  Despite the name, this is a general synced-write helper: anything that
  needs a `Cachex.put/4` to converge across nodes can use it, not just rate
  limiters. Rate limiters (`Ysc.ResendRateLimiter`, `Ysc.SmsRateLimit`, ...)
  have no database to fall back to, so a plain invalidate-and-refetch
  pattern doesn't help across nodes — each node's cache is the only copy of
  that state. Event cache version keys (`Ysc.Events.EventPricingCache`,
  `Ysc.Events.EventListCache`, `Ysc.PublicContentCache`) have the same
  problem: bumping the version locally only makes that node see the new
  data, so other nodes keep serving stale cached values until TTL expiry.
  Broadcasting the write itself keeps every node's local cache converged
  instead.
