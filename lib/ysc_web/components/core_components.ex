defmodule YscWeb.CoreComponents do
  ...
  attr :rest, :global,
    include:
      ~w(phx-click phx-target phx-value-id phx-value-payment_id phx-value-webhook_id phx-value-expense_report_id phx-value-user_id phx-value-invite_id phx-value-tier-id phx-value-email phx-value-booking-id phx-disable-with data-confirm disabled target rel aria-label)
  ...
  @account_settings_nav_keys [
    ...
    {:payments, "My Bookings & Payments", "hero-wallet"},
  ...
  """
  def close_dropdown(to), do: hide_dropdown(to)

  @spec translate_error({binary(), keyword() | map()}) :: binary()
  @doc """
  Translates an error message using gettext.
  """
  def translate_error(error), do: FormHelpers.translate_error(error)
          ...
          <div class="ml-3">
            ...
            <p class="text-sm text-red-700">
              <%= if @is_sub_account do %>
                ...
                <% else %>
                  The member who manages your family account does not have an active membership. You need an active family membership to access YSC events and benefits.
                <% end %>
              <% else %>
  ...
  `phx-click`, `phx-target`, and related global attributes.
  ...
  ## Examples
      ...
      />
  """
  attr :provider, :atom, required: true, values: [:google, :facebook]
  attr :label, :string, required: true
  attr :class, :string, default: nil

  attr :rest, :global,
    include: ~w(phx-click phx-target phx-disable-with disabled id)
  ...
  def oauth_button(assigns) do
    assigns =
      assign(
        ...
        :button_class,
        [
          "w-full flex items-center justify-center gap-2 h-10 border-zinc-300 text-zinc-700",
          assigns[:class]
        ]
    ...
    <.button
      ...
      {@rest}
    >
      <.oauth_provider_icon provider={@provider} />
      {@label}
    </.button>
    """
  end
  ...
  defp oauth_provider_icon(%{provider: :google} = assigns) do
    ~H"""
    <svg
  ...
  defp oauth_provider_icon(%{provider: :facebook} = assigns) do
    ~H"""
    <svg
      xmlns="http://www.w3.org/2000/svg"
      height="20"
