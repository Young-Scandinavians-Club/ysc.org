defmodule Ysc.Bookings.BookingLockerTest do
  ...
  describe "revert_hold_to_draft/1" do
    test "reverts buyout hold to draft and clears buyout_held", %{user: user} do
      {checkin, checkout} = locker_buyout_dates(420)
      ...
      assert property_buyout_held?(:tahoe, days)

      assert {:ok, reverted} = BookingLocker.revert_hold_to_draft(booking.id)
      assert reverted.status == :draft
      refute property_buyout_held?(:tahoe, days)
    end

    test "reverts room hold to draft and clears held inventory", %{user: user} do
      {:ok, _} =
        Bookings.create_pricing_rule(%{
          amount: Money.new(:USD, 100),
          booking_mode: :room,
      ...
      {:ok, room} =
        Bookings.create_room(%{
          name: "Locker revert hold room",
          property: :tahoe,
          room_category_id: category.id,
          capacity_max: 4
        })

      {checkin, checkout} = locker_room_dates(421, 2)
    ...
    test "reverts day hold to draft and releases capacity_held", %{user: user} do
      ensure_clear_lake_day_pricing_rule()

      {checkin, checkout} = locker_room_dates(422, 2)
      guests = 3
      ...
      before =
        Repo.get_by!(PropertyInventory, property: :clear_lake, day: day)
  ...
  describe "revert_complete_to_draft/1" do
    test "reverts complete buyout to draft and clears buyout_booked", %{
      user: user
    } do
    ...
    test "reverts complete room booking to draft and clears booked inventory",
         %{
           user: user
         } do
      {:ok, _} =
      ...
      {:ok, room} =
        Bookings.create_room(%{
          name: "Locker revert complete room",
          property: :tahoe,
          room_category_id: category.id,
          capacity_max: 4
        })

      {checkin, checkout} = locker_room_dates(425, 2)
    ...
    test "reverts complete day booking to draft and releases capacity_booked",
         %{
           user: user
         } do
      ensure_clear_lake_day_pricing_rule()

      {checkin, checkout} = locker_room_dates(426, 2)
      guests = 3
      ...
      assert after_revert.capacity_booked == before.capacity_booked - guests
    end

    test "returns invalid_status when booking is still a hold", %{user: user} do
      {checkin, checkout} = locker_buyout_dates(427)
  ...
  describe "release_hold_with_stripe_reconcile/1" do
    ...
    test "confirms the hold when Stripe cancel reveals payment already succeeded",
      ...
      payment_intent_id =
        "pi_release_hold_succeeded_#{System.unique_integer([:positive])}"
      ...
      try do
        ...
        assert {:confirmed, confirmed} =
                 BookingLocker.release_hold_with_stripe_reconcile(hold.id)
    ...
    test "keeps the hold when PaymentIntent is still processing", %{user: user} do
      {checkin, checkout} = locker_buyout_dates(421)
      ...
      payment_intent_id =
        "pi_release_hold_processing_#{System.unique_integer([:positive])}"
    ...
    test "releases an unpaid hold after Stripe confirms the Intent canceled",
         %{user: user} do
      {checkin, checkout} = locker_buyout_dates(422)
      ...
      payment_intent_id =
        "pi_release_hold_unpaid_#{System.unique_integer([:positive])}"
    ...
    test "keeps the hold when Stripe cancel times out", %{user: user} do
      alias Ysc.Bookings.Entitlements

      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 425)
    ...
    test "refunds and releases the hold when succeeded payment amount does not match",
         %{user: user} do
      alias Ysc.Bookings.Entitlements

      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 426)
      ...
      try do
        ...
        assert canceled.status == :canceled
        assert is_nil(canceled.applied_booking_entitlement_id)
    ...
    test "keeps the hold when succeeded payment metadata does not match", %{
      user: user
    } do
      alias Ysc.Bookings.Entitlements

      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 427)
    ...
    test "keeps the hold when payment succeeded but booking confirmation fails",
         %{user: user} do
      alias Ysc.Bookings.Entitlements

      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 428)

      # Consume will fail (entitlement no longer active) while inventory is
      # still held, so a mistaken :release would cancel the paid hold.
    ...
    test "releases the hold when Stripe refused cancel because the PI is already canceled",
      ...
      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 429)
  ...
  describe "revert_hold_to_draft_with_stripe_reconcile/1" do
    ...
    test "keeps the hold when Stripe cancel times out", %{user: user} do
      ...
      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 440)
    ...
    test "refunds and reverts the hold to draft when succeeded payment amount does not match",
         %{user: user} do
      alias Ysc.Bookings.Entitlements

      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 441)
      ...
      try do
        ...
        assert reverted.status == :draft
        assert is_nil(reverted.applied_booking_entitlement_id)
    ...
    test "keeps the hold when succeeded payment metadata does not match", %{
      ...
      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 442)
    ...
    test "keeps the hold when payment succeeded but booking confirmation fails",
      ...
      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 443)

      # Consume will fail (entitlement no longer active) while inventory is
      # still held, so a mistaken :release would drop the paid hold to draft.
      assert {:ok, _} = Entitlements.revoke_entitlement(entitlement)
    ...
    test "reverts the hold to draft when Stripe refused cancel because the PI is already canceled",
         %{user: user} do
      alias Ysc.Bookings.Entitlements

      {hold, payment_intent_id, entitlement} =
        hold_for_release_reconcile(user, 444)
  ...
  describe "confirm_booking/1 other holds" do
    ...
    test "confirms a sibling hold when Stripe cancel reveals payment already succeeded",
         %{user: user} do
      alias Ysc.Bookings.Entitlements

      {week1_in, week1_out} = locker_buyout_dates(30)
      {week2_in, week2_out} = locker_buyout_dates_after(week1_out)
      ...
      payment_intent_id =
        "pi_sibling_hold_succeeded_#{System.unique_integer([:positive])}"
      ...
      try do
        ...
        consumed = Entitlements.get_entitlement(entitlement.id)
        assert consumed.status == :consumed
        assert consumed.consumed_booking_id == sibling.id

        payment = Ysc.Ledgers.get_payment_by_external_id(payment_intent_id)
        assert payment
        assert payment.status == :completed
        assert Money.equal?(payment.amount, second_hold.total_price)
      after
        Application.put_env(:ysc, :stripe_client, previous_client)
      end
    end
    ...
    test "keeps a sibling hold when its PaymentIntent is still processing", %{
      ...
      {week1_in, week1_out} = locker_buyout_dates(31)
      {week2_in, week2_out} = locker_buyout_dates_after(week1_out)
      ...
      payment_intent_id =
        "pi_sibling_hold_processing_#{System.unique_integer([:positive])}"
        ...
        expect(Ysc.StripeMock, :retrieve_payment_intent, fn ^payment_intent_id,
          ...
          {:ok,
           %Stripe.PaymentIntent{
             ...
             status: "processing",
             amount: 10_000,
             metadata: %{"booking_id" => second_hold.id}
           }}
        end)
    ...
    test "releases an unpaid sibling hold after Stripe confirms the Intent canceled",
         %{user: user} do
      {week1_in, week1_out} = locker_buyout_dates(32)
      {week2_in, week2_out} = locker_buyout_dates_after(week1_out)
      ...
      payment_intent_id =
        "pi_sibling_hold_unpaid_#{System.unique_integer([:positive])}"
  ...
  # Helper functions
  defp property_buyout_held?(property, days) do
    Enum.all?(days, fn day ->
      Repo.one!(
        from(pi in PropertyInventory,
          where: pi.property == ^property and pi.day == ^day,
  ...
  defp property_buyout_booked?(property, days) do
    Enum.all?(days, fn day ->
      Repo.one!(
        from(pi in PropertyInventory,
          where: pi.property == ^property and pi.day == ^day,
  ...
  defp hold_for_release_reconcile(user, slot) do
    alias Ysc.Bookings.Entitlements
    ...
    payment_intent_id =
      "pi_release_hold_edge_#{slot}_#{System.unique_integer([:positive])}"
