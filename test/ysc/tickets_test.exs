defmodule Ysc.TicketsTest do
  ...
  describe "pending order complimentary recalculation (#604)" do
    ...
    test "pending_order_still_complimentary?/1 is false for a partial reservation discount",
         %{user: user, event: event, tier1: tier1} do
      %Ysc.Events.TicketReservation{}
      |> Ysc.Events.TicketReservation.changeset(%{
        ticket_tier_id: tier1.id,
      ...
      {:ok, expected_total} = Money.sub(tier1.price, Money.new(25, :USD))
      ...
      assert Money.equal?(total, expected_total)
      assert Money.equal?(discount, Money.new(25, :USD))
    end
