defmodule YscWeb.AdminFlopHelpersTest do
  ...
  describe "title_search_query/1" do
    ...
    test "returns empty string when no title filter" do
      meta = %{flop: %{filters: [%Flop.Filter{field: :state, op: :==, value: "draft"}]}}
  ...
  describe "merge_date_range_into_params/3" do
    test "adds non-empty date range keys" do
      assert AdminFlopHelpers.merge_date_range_into_params(%{"tab" => "all"}, "2026-01-01", "") ==
               %{"tab" => "all", "date_from" => "2026-01-01"}

      assert AdminFlopHelpers.merge_date_range_into_params(%{}, "", "2026-12-31") == %{
               "date_to" => "2026-12-31"
             }
    end

    test "skips empty date range keys" do
      assert AdminFlopHelpers.merge_date_range_into_params(%{"tab" => "all"}, "", "") == %{
               "tab" => "all"
             }
    end
  end
  ...
  describe "compact_filter_params/1" do
    test "drops empty values and normalizes multi-select blanks" do
      assert AdminFlopHelpers.compact_filter_params(%{
               "0" => %{"field" => "state", "op" => "==", "value" => "published"},
  ...
  describe "merge_title_filter_into_params/2" do
    test "re-applies active title filter from meta" do
      meta = %{flop: %{filters: [%Flop.Filter{field: :title, op: :ilike, value: "keep"}]}}

      assert AdminFlopHelpers.merge_title_filter_into_params(%{"0" => %{"field" => "state"}}, meta) ==
               %{
                 "0" => %{"field" => "state"},
                 "1" => %{"field" => "title", "op" => "ilike", "value" => "keep"}
               }
    end
    ...
    test "leaves params unchanged when title filter is absent or empty" do
      ...
      assert AdminFlopHelpers.merge_title_filter_into_params(updated, nil) == updated

      meta = %{flop: %{filters: [%Flop.Filter{field: :title, op: :ilike, value: ""}]}}

      assert AdminFlopHelpers.merge_title_filter_into_params(updated, meta) == updated
    end
  end
end
