defmodule YscWeb.EventDetailsLive.PaymentFlowTest do
  ...
  describe "payment modal UI state" do
    ...
    test "close-payment-modal keeps pending order when payment intent requires action",
         %{
           conn: conn,
           user: user
         } do
      ...
      expect_payment_requires_action(payment_intent_id)
    ...
    test "close-payment-modal keeps pending order when payment intent requires confirmation",
         %{
           conn: conn,
           user: user
         } do
      ...
      expect_payment_requires_confirmation(payment_intent_id)
  ...
  describe "payment intent refresh on checkout restore (#610)" do
    ...
    test "re-syncs payment intent when tier price changes via pubsub during open checkout",
         %{
           conn: conn,
           user: user
         } do
      Oban.Testing.with_testing_mode(:manual, fn ->
        ...
        old_amount_cents = money_to_cents(order.total_amount)
        pi_id = "pi_stale_pubsub_#{System.unique_integer([:positive])}"
        new_price = Money.mult!(order.total_amount, 2)
        new_amount_cents = money_to_cents(new_price)
        ...
        expect(Ysc.StripeMock, :create_payment_intent, fn params, _opts ->
          ...
          {:ok,
           build_payment_intent(%{
             id: "pi_pubsub_repriced_#{System.unique_integer([:positive])}",
             amount: new_amount_cents
           })}
        end)
        ...
        view = wait_for_async(view)
        assert has_element?(view, "#payment-modal")
        assert render(view) =~ "Prices updated"
      end)
    end

    test "does not cancel payment intent when tier reprices during 3DS",
         %{
           conn: conn,
           user: user
         } do
      Oban.Testing.with_testing_mode(:manual, fn ->
        ...
        old_amount_cents = money_to_cents(order.total_amount)
        pi_id = "pi_3ds_in_flight_#{System.unique_integer([:positive])}"
        new_price = Money.mult!(order.total_amount, 2)
        ...
        reloaded = Tickets.get_ticket_order(order.id)
        assert money_to_cents(reloaded.total_amount) == old_amount_cents
      end)
    end
  end
