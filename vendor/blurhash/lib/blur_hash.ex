defmodule BlurHash do
  ...
  Vendored from hex `blurhash` 2.0.0 (https://github.com/perzanko/blurhash-elixir,
  MIT licensed — see LICENSE.md) with one change: `encode_base83/2` builds its
  descending range with an explicit `//-1` step so it compiles cleanly under
  newer Elixir versions instead of emitting a `Range.new/2` deprecation
  warning on every compile. No upstream release includes this fix.
  ...
  def encode(pixels, width, height, x_components, y_components) do
    ...
    factors =
      calculate_factors(pixels, width, height, x_components, y_components)
    ...
    ac_encoded =
      Enum.map(ac, fn component -> encode_ac(component, max_ac_value) end)

    ac_hash =
      Enum.map(ac_encoded, fn value -> encode_base83(value, 2) end)
      |> Enum.join()
    ...
    for y <- 0..(height - 1), x <- 0..(width - 1) do
      {r, g, b} =
        ...
        |> Enum.reduce({0.0, 0.0, 0.0}, fn {{color_r, color_g, color_b}, index},
          ...
          {acc_r + color_r * basis, acc_g + color_g * basis,
           acc_b + color_b * basis}
        end)
  ...
  defp multiply_basis_function(pixels, width, height, x_component, y_component) do
    ...
    |> Enum.chunk_every(3)
    |> Enum.with_index()
    |> Enum.reduce({0.0, 0.0, 0.0}, fn {[r, g, b], pixel_index},
                                       {acc_r, acc_g, acc_b} ->
      ...
      {acc_r + basis * linear_r, acc_g + basis * linear_g,
       acc_b + basis * linear_b}
    end)
  end
  ...
  defp encode_base83(value, length) do
    {result, _} =
      Enum.reduce((length - 1)..0//-1, {[], value}, fn i, {acc, val} ->
